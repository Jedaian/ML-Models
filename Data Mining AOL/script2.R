...
rf_model <- randomForest(Survived ~ Pclass + Sex + Age + Embarked + Title 
                         + Alone + Fare, data = train, ntree = 500)
...
#Logistic Regression
logistic <- glm(Survived ~ Pclass + Sex + Age + Embarked + Title 
                + Alone + Fare, family = binomial, data = train)
...
library(e1071)
svm_model <- svm(formula = Survived ~ ., data = train, 
...
#RF with PCA
library(stats)
pca <- prcomp(train[, -which(names(train) == "Survived")], 
